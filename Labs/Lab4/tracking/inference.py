    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for current_position, current_probability in self.beliefs.items():
            new_distribution = self.getPositionDistribution(self.setGhostPosition(gameState, current_position))
            for new_position, new_probability in new_distribution.items():
                updated_beliefs[new_position] = updated_beliefs[new_position] + new_probability * current_probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(1, self.numParticles + 1):
            index  = i % len(self.legalPositions)
            self.list_of_particles.append(self.legalPositions[index])
        # self.beliefs = util.Counter()
        # initial_weight = 0
        # self.beliefs ={position: initial_weight for position in self.legalPositions}
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            # throw out the particles list
            # setting particls position to the jail position
            self.list_of_particles = [self.getJailPosition()] * self.numParticles
        else:
            beliefs = util.Counter()
            for particle in self.list_of_particles:
                distance = util.manhattanDistance(pacmanPosition, particle)
                beliefs[particle] += emissionModel[distance]
            #self.list_of_particles = [util.sample(beliefs) for i in self.particles]
            if all(i == 0 for i in beliefs.values()):
                self.initializeUniformly(gameState)
            else:
                self.list_of_particles = [util.sample(beliefs) for i in self.list_of_particles]
    ...
    def elapseTime(self, gameState):
        ...
        a belief distribution.
        """
        for index, p in enumerate(self.list_of_particles):
            self.list_of_particles[index] = util.sample(
                self.getPositionDistribution(self.setGhostPosition(gameState, p)))
        #print(self.list_of_particles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #     index  = i % len(source)
        #     self.list_of_particles.append(source[index])
        self.list_of_particles = [source[i % len(source)] for i in xrange(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        for index, distance in enumerate(noisyDistances):
            if distance is None:
                self.list_of_particles = [self.getParticleWithGhostInJail(p, index)
                                  for p in self.list_of_particles]
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.list_of_particles:
        ...
        self.list_of_particles = newParticles
